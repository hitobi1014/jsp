...
public class RedirectServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// servlet은 응답을 만들어 내는 역할이 아니라 요청을 받고 로직을 처리한 후 jsp에게 화면 응답 생성을 위임한다.

		// 문자열 결합만 조심 => "redirectServlet" "doGet()"
//		logger.debug("redirectServlet{} {} ", "doGet()", "test");
		logger.debug("redirectServlet.doGet()");
		// 응답을 다른 jsp에게 위임하는 첫번째 방법 : redirect
		// response 객체의 sendRedirect 메소드를 통해 클라이언트에게 재요청을 보낼 주소를 알려준다
		...
		// response객체를 이용하여 다른 jsp파일로 클라이언트에게 재요청 보내줄것을 안내
		// 클라이언트가 최초 요청에 대한 응답(redirect 응답)을 받고서 서버의 안내대로
		// http://localhost/redirect/delegate/redirectView.jsp로 재요청
		// 그래서 최종적으로 클라이언트의 주소창에는 jsp요청에 대한 주소가 남는다.
		// 클라이언트 입장에서는 서버로 요청을 총 두 번 보내게 된다

		// 아래 데이터를 db에서 조회한 데이터라 생각
		...
		// 서버가 redirect로 응답을 할 경우 클라이언트는 해당주소로 새로운 요청을 보내기 때문에 servlet에서 request 스코프에 설정한 속성은
