...
public class YogurtBatchApplication {
	...
	public static void main(String[] args) {
		...
		new ClassPathXmlApplicationContext("classpath:kr/or/ddit/config/spring/datasource-context.xml",
											"classpath:kr/or/ddit/config/spring/transaction-context.xml",
											"classpath:kr/or/ddit/config/spring/yogurt-batch.xml");

		//jobLauncher, job 스프링빈 DL
		JobLauncher jobLauncher = context.getBean("jobLauncher",JobLauncher.class);
		Job yogurtJob = context.getBean("yogurtJob", Job.class);
		
		try {
			jobLauncher.run(yogurtJob, new JobParametersBuilder()
								.addParameter("ym", new JobParameter("202012"))
								.addLong("st_ dt", System.currentTimeMillis()).toJobParameters());
