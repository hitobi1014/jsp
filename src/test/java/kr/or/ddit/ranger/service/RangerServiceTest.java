...
public class RangerServiceTest {
	...
	// 하나의 메소드도 시나리오에 따라서는 여러개의 테스트 메소드를 생성할 수 도 있다
	// ex : 회원등록 dao 메소드 생성 시 
	//		정상적인 값을 입력해서 db 테이블에 데이터가 정상적으로 입력되는 성공 케이스
	//		비정상적인 값 (회원 id 중복)을 입력해서 db 테이블에 데이터가 입력되지 않는 실패 케이스
	...
	// 이클립스에서 @Test메소드가 붙은 메소드를 실행 해주기 때문(junit에서 실행)
